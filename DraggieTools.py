    ...
    with open(f'{current_directory}\\DraggieTools-{current_build_version}.exe', 'wb') as f:
        for chunk in r.iter_content(chunk_size=1024):
            ...
            if now - last_print > 0.1:
                ...
                print(Fore.LIGHTCYAN_EX + f'Downloading. {pct_done}% done, avg speed {speed} kbps')
                last_print = now
...
def check_for_update():
    ...
    if build < current_build_version:
        update_choice = input(Fore.RED + f"Update available! You are on version {version} which is build {build}. The newest version is build {current_build_version}\n\nType 1 to download update, else not\n\n>>> ")
        if update_choice == "1":
